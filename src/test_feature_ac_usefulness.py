...
def test(model_name, iter_num, gpu_id, n_test, filename, vol_size=(160,192,224), nf_enc=[16,32,32,32], nf_dec=[32,32,32,32,32,16,16]):
    ...
    for step in range(0, n_test):
        ...
        with tf.device(gpu):
            ...
            warped_image = pred[0][:, :, :, :, :]
            pred_ac_features = autoencoder.predict([warped_image])[1:]
            orig_ac_features = autoencoder.predict([X_vol])[1:]
        ...
        for i in range(len(pred_ac_features)):
            normalized_pred = normalize_percentile(pred_ac_features[i], percentile, feature_stats, i)
            normalized_orig = normalize_percentile(orig_ac_features[i], percentile, feature_stats, i)
